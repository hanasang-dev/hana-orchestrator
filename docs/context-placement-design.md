...
```
ChatRequest(message) → ChatDto(message) → Orchestrator.executeOrchestration(query)
  → OrchestrationCoordinator.executeOrchestration(query)
  → createInitialTree(query, allDescriptions, ...)
  → LLMClient.createExecutionTree(userQuery, layerDescriptions)  // 여기서 트리 생성
  → LLMPromptBuilder.buildExecutionTreePrompt(userQuery, layerDescriptions)
  → 트리 실행 (TreeExecutor)
```

**중요:** 트리가 **생성되기 전에** LLM은 `userQuery`와 `layerDescriptions`만 받는다.  
**범용 컨텍스트**를 받아서 **항상** 이 단계의 프롬프트에 한 블록으로 넣으면, LLM이 “이 요청에 대한 추가 정보를 일관되게 활용할 수 있다.
