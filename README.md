...
│   │   ├── EchoLayer.kt            # 예제 레이어
│   │   ├── LLMLayer.kt             # LLM 직접 답변 레이어
│   │   ├── TextTransformerLayer.kt # 텍스트 변환 레이어
│   │   ├── TextValidatorLayer.kt   # 텍스트 검증 레이어
│   │   ├── FileSystemLayer.kt      # 파일 시스템 작업 레이어
│   │   ├── LayerInfoLayer.kt       # 레이어 정보 조회 레이어
│   │   └── LayerFactory.kt         # 레이어 생성 도구
│   ├── orchestrator/               # 오케스트레이터 구현
│   │   ├── Orchestrator.kt        # 핵심 오케스트레이터
│   │   ├── OrchestrationCoordinator.kt  # 실행 조정
│   │   ├── TreeExecutor.kt        # 실행 트리 실행
│   │   └── ExecutionTreeValidator.kt
│   ├── llm/                        # LLM 클라이언트 (오케스트레이터 전용)
│   │   ├── OllamaLLMClient.kt      # Ollama LLM 클라이언트
│   │   ├── LLMPromptBuilder.kt    # 프롬프트 빌더
│   │   ├── JsonSchemaBuilder.kt   # JSON Schema 생성 (Structured Outputs)
│   │   ├── JsonExtractor.kt        # JSON 추출 유틸리티
│   │   └── LLMClient.kt            # LLM 클라이언트 인터페이스
│   ├── domain/                     # 도메인 엔티티
│   │   └── entity/
│   │       ├── ExecutionTree.kt
│   │       ├── ExecutionNode.kt
...
│   └── Dockerfile.*              # 다양한 모델용 Dockerfile
├── docs/                          # 문서
│   ├── ai-agent-best-practices.md  # AI Agent 모범 사례
│   ├── nlp-vs-structured-output-analysis.md  # NLP vs Structured Output 분석
│   ├── structured-output-implementation-2026.md  # Structured Output 구현 가이드
│   └── ...
...
- **JsonSchemaBuilder 추가**: Ollama Structured Outputs를 위한 JSON Schema 생성
  - DRY 원칙 적용: 헬퍼 함수로 중복 코드 제거
  - 프로바이더 독립적 설계로 향후 다른 LLM 클라이언트에도 재사용 가능
- **LLM 클라이언트 개선**:
  - JSON 파싱 실패 시 자동 재시도 로직 (최대 2회)
  - 재시도 시 에러 정보를 프롬프트에 포함하여 개선
  - schema 파라미터 추가 (향후 Structured Outputs 완전 지원 준비)

### FileSystemLayer 추가
- 파일 읽기/쓰기, 디렉토리 목록 조회, 파일 검색, 백업 기능 제공
- 상대 경로 사용 강제로 경로 해석 오류 방지
- 자동 백업 기능으로 파일 수정 안전성 보장

### LLM 프롬프트 개선
- **경로 처리 규칙 추가**: 상대 경로 사용 강제로 절대 경로 오류 방지
- **Generic 예시**: 하드코딩된 레이어/함수 이름 제거로 LLM의 독립적 판단 유도
- **JSON 필수 필드 강조**: 필드 누락으로 인한 파싱 오류 방지
